...
def genNoise(size=64):
    return cp.random.choice([0., 1.], (size,size))
...
def genLabeledDataset(n, size=64): # 0.5 triangle, 0.25 square, 0.25 noise
    ...
    for _ in range(n):
        ...
        if c >> 1:
            ...
            labels.append(0)
    return cp.array(data), cp.array(labels)[:, cp.newaxis, cp.newaxis] # this choice is because we are doing binary classification, last layer only have 1 neuron
...
def sigmoid(x, deriv=False):
    sigmoid_ = cp.reciprocal(1 + cp.exp(-x))
    if deriv:
...
def tanh(x, deriv=False):
    if deriv:
        return cp.reciprocal(2 * cp.cosh(x)**2)
...
def main():
    ...
    w1 = cp.random.randn(n1, isize**2)
    b1 = cp.random.randn(n1, 1)
    w2 = cp.random.randn(n2, n1)
    b2 = cp.random.randn(n2, 1)
    w3 = cp.random.randn(1, n2)
    ...
    while True:
        for batch in range(nbatch):
            print('Batch #' + str(batch) + '/' + str(nbatch))
...
##            print('--- Preprocessing images ---')
            ...
            a1s = cp.array([activate(w1 @ a0 + b1) for a0 in a0s])
            a2s = cp.array([activate(w2 @ a1 + b2) for a1 in a1s])
            a3s = cp.array([activate(w3 @ a2 + b3) for a2 in a2s])

            # the errors
            oldError, error = error, (cp.array([round(float(a3)) for a3 in a3s.reshape(-1)]) ^ labels.reshape(-1)).sum() / dsize
            ...
            db3s = cp.array([2 * (a3 - y) * activate(w3 @ a2 + b3, deriv=True) for a2,a3,y in zip(a2s,a3s,labels)])
            dw3s = cp.array([db3 * a2.T for a2,db3 in zip(a2s,db3s)])
            da2s = cp.array([w3.T @ db3 for db3 in db3s])

            db2s = cp.array([da2 * activate(w2 @ a1 + b2, deriv=True) for a1,da2 in zip(a1s,da2s)])
            dw2s = cp.array([db2 * a1.T for a1,db2 in zip(a1s,db2s)])
            da1s = cp.array([w2.T @ db2 for db2 in db2s])

            db1s = cp.array([da1 * activate(w1 @ a0 + b1, deriv=True) for a0,da1 in zip(a0s,da1s)])
            dw1s = cp.array([db1 * a0.T for a0,db1 in zip(a0s,db1s)])
